...
protected: 
	...
	void bubbleSort_v1 ( Rank lo, Rank hi ); //起泡排序算法 
	void bubbleSort_v2 ( Rank lo, Rank hi ); //起泡排序算法 
	...
	void quickSort_v1 ( Rank lo, Rank hi ); //快速排序算法 
	void quickSort_v2 ( Rank lo, Rank hi ); //快速排序算法
...
template <typename T> 
Vector<T> &  Vector<T>::operator= ( Vector<T> const& ){//重载赋值操作符，以便直接克隆向量 
	if (this != &V) {
        delete[] _elem;
...
void  Vector<T>::sort ( Rank lo, Rank hi ){//对[lo, hi)排序 
	switch (rand() % 6){
		...
		default: shellSort ( Rank lo, Rank hi );break;
	}
} 
...
template <typename T> 
void Vector<T>::bubbleSort_v1(Rank lo, Rank hi){
	while(!bubble(lo,hi--));
}
template <typename T>
void Vector<T>::bubbleSort_v2(Rank lo, Rank hi){
...
// [3]轴点构造算法
//----------------------------------
template <typename T> void Vector<T>::quickSort_v1 ( Rank lo, Rank hi ){//快速排序算法
...
template <typename T> void Vector<T>::quickSort_v2(Rank lo, Rank hi){
	Stack<Rank> Task;//
	Put(Task, lo, hi);
	while(!Task.empty){
...
template <typename T> Rank Vector<T>::partition(Rank lo, Rank hi){
	swap(_elem[lo], _elem[lo+rand()%(hi-lo)]);//随机抽取作为轴点
	T pivot = _elem[lo];//取出轴点作备份
