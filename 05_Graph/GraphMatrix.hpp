...
private:
    Vector<Vertex<Tv>> V; //顶点集（向量）向量里面装的是Vertex对象。
    Vector<Vector<Edge<Te>*>> E; //边集（邻接矩阵）第一层向量里面装的是向量，第二层向量里面装的是指向边对象的指针。
public:
...
// 顶点的基本操作：查询第v个顶点（0 <= v < n）
    ...
    virtual Rank& parent( Rank v ) { return V[v].parent; } //在遍历树中的父亲，返回的是在向量中的秩
